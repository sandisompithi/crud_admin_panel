package pnp.admin_crud.dao;

import java.util.List;

import org.hibernate.Session;
import org.hibernate.SessionFactory;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Repository;

import pnp.admin_crud.model.Product;

@Repository(value="productDao")
public class ProductDaoImpl implements ProductDao {

	
	// this class is wired with the sessionFactory to 
		// do some operation in the database
			
	@Autowired
	private SessionFactory sessionFactory;
	
	// this will create one sessionFactory for this class
	// there is only one sessionFactory should be created for the applications
	// we can create multiple sessions for a sessionFactory
	// each session can do some functions
	public SessionFactory getSessionFactory() {
		return sessionFactory;
	}

	public void setSessionFactory(SessionFactory sessionFactory) {
		this.sessionFactory = sessionFactory;
	}
	
	public List<Product> getAllProducts() {
		
		// Read all the records from the table
		Session session = this.sessionFactory.openSession();
		
		List<Product> products = session.createCriteria(Product.class).list();
		System.out.println("All the products");
		System.out.println(products);
				
		// session.flush is used for clear cache in the session
		session.flush();
				
		// close the particular session after completing the process
		session.close();
				
		return products;
	}

	public Product getProductById(String productId) {
		
		// Read all the records from the table
		Session session = this.sessionFactory.openSession();
		
		// select * from Product where prodId=i
		Product product = (Product)session.get(Product.class, productId);
		
		// close the particular session after completing the process
		session.close();
				
		return product;
		
	}

	public void addProduct(Product product) {
		
		Session session = this.sessionFactory.openSession();
		
		session.save(product);
		
		session.close();
		
	}

	public void updateProduct(Product product) {
		
		Session session = this.sessionFactory.openSession();
		
		session.update(product);
		
		// clear cache in the session
		session.flush();
		
		session.close();
		
	}

	public void deleteProduct(String productId) {
		
		Session session = this.sessionFactory.openSession();
		
		session.delete(productId);
		
		session.flush();
		
		session.close();
		
	}

	
}

